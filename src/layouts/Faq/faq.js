...
class Faq extends Component {
  ...
  handleSubmit = async (e) => {
    e.preventDefault();
    const { formData, currentFaqId } = this.state;
    const token = localStorage.getItem("admin_token");
    const url = currentFaqId
      ? `http://34.47.154.170/api/admin/faq/edit/${currentFaqId}`
      : "http://34.47.154.170/api/admin/faq/create";
    ...
    try {
      ...
      Swal.fire({
        position: "top-end",
        icon: "error",
        title: "Failed to save FAQ. Please try again.",
        showConfirmButton: false,
        timer: 3000,
      });
    }
  ...
  handleDelete = async (faqId) => {
    ...
    try {
      ...
      Swal.fire({
        position: "top-end",
        icon: "error",
        title: "Failed to delete FAQ. Please try again.",
        showConfirmButton: false,
        timer: 3000,
      });
    }
  ...
  handleEdit = (id) => {
    const faq = this.state.faqs.find((item) => item._id === id);
    if (faq) {
      this.setState({
        formData: {
          question: faq.question,
  ...
  render() {
    const { show, formData, faqs } = this.state;

    return (
      <DashboardLayout>
        <DashboardNavbar />
        <div>
          <Button variant="primary" onClick={this.handleShow} style={{ marginBottom: "10px" }}>
            Add FAQ
          </Button>

          <Modal show={show} onHide={this.handleClose} centered>
            <Modal.Header closeButton>
              <Modal.Title>{this.state.currentFaqId ? "Edit FAQ" : "Add FAQ"}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <Form onSubmit={this.handleSubmit}>
                ...
                <Modal.Footer>
                  <Button variant="primary" type="submit">
                    {this.state.currentFaqId ? "Update" : "Submit"}
                  </Button>
                ...
                <Card style={{ height: "100%", backgroundColor: "#f8f9fa" }}>
                  <Card.Body>
                    ...
                    <div className="d-flex">
                      <Button
                        variant="dark"
                        onClick={() => this.handleEdit(faq._id)}
